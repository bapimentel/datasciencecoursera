...
As expected, Random Forest obtained better result than Decision Tree, since Random Forest used 200 tree to make the prediction. 
...
From this figure, it is possible to highlight that "roll_belt", "pitch_belt" and "yaw_belt" are the most relevante to prediction of Random Forest model. The following figure shows the Out of Bag (OOB) score in Random Forest model according to variation of number of trees.
...
As expected, for small number of tree, the error is higher. However, the error decreases as the number of trees increases. Also, it is possible to conclude that the error stabilizes when the number of trees is arround 50. It is also important to highlight that the error for class "E" is higher than error for class "C", whatever the number of trees used in the Random Forest model. The following fire shows the dispersion of measures from training data regarding variables "roll_belt" and "yaw_belt".
